...
function CourseCard({ course }: { course: Course }) {
  return (
    <article
      className="relative bg-[#0f1420] border border-white/5 rounded-2xl shadow-lg transition-all duration-200 hover:ring-1 hover:ring-white/10 hover:-translate-y-0.5 overflow-hidden"
      aria-labelledby={`course-${course.id}-title`}
    >
      {/* Gradient strip on the left */}
      <div className="absolute left-0 top-0 bottom-0 w-14 md:w-[72px] bg-gradient-to-b from-[#FF84B7] via-[#FF7AD9] to-[#B36BFF]" />
      ...
      <div className="ml-14 md:ml-[72px] p-4 md:p-5 flex flex-col h-full">
        ...
        <div className="flex justify-end">
          <button
            className="bg-[#6D3BFF] hover:bg-[#7B4DFF] text-white text-xs md:text-sm px-4 py-1.5 rounded-full transition-colors duration-200 flex items-center gap-2"
...
export default function StudentCourses() {
  return (
    <div className="min-h-screen bg-[#070B13] text-white/90">
